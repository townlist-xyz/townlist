...
app.get("/bot/:botID/edit", global.checkAuth, async (req, res) => {
    ...
    if (!botdata) return res.redirect("/error?code=404&message=You entered an invalid bot.")
...
app.post("/bot/:botID/edit", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(req.params.botID).then(a => {
        client.channels.cache.get(channels.botlog).send(`${emoji.pen} <@${req.user.id}> edited **${a.tag}**`)
