...
app.post("/bot/:botID/vote", global.checkAuth, async (req, res) => {
    ...
    client.channels.cache.get(channels.votes).send(new MessageEmbed()
    .setTitle(`${emoji.mention} Voting System`)
    .addField(`Voted By`, `${req.user.username}`)
    .addField(`Total Votes`, `${botdata.votes + 1}`)
    .setDescription(`**${botdata.username}** just got voted by a member.\nVotes reset per month.`)
    .setColor(`2f3136`)
    )
    if(botdata.votes+1 == 100) {
    client.channels.cache.get(channels.votes).send(new MessageEmbed()
    .setTitle(`${emoji.mention} Milestone`)
    .setDescription(`Congratulations ${botdata.ownerID}.\nYour bot **${botdata.username}** has reached 100 votes!`)
    .setColor('2f3136')
    )
    }
