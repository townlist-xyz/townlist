...
app.post("/admin/maintence", global.checkAuth, async (req, res) => {
    ...
    client.channels.cache.get(global.config.server.channels.webstatus).send(`${emoji.off} DTL has been switched to __Maintenance__ due to **${req.body.reason}** [||<@&861221279080120371>||]`).then(a => {
        new maintenceSchema({
            server: config.server.id,
...
app.post("/admin/unmaintence", global.checkAuth, async (req, res) => {
    ...
    const bakimsonaerdikardesdisbots = new dc.MessageEmbed()
        .setAuthor("https://townlist.xyz", client.user.avatarURL())
        .setThumbnail(client.user.avatarURL())
        .setColor("GREEN")
        .setDescription(`${emoji.success} DTL is **active** again!\n[Click to redirect website](https://townlist.xyz)`)
        .setFooter("DTL © All rights reserved.");
    await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => {
        a.edit(`~~ ${emoji.offline} DTL has been switched to __maintance__ due to **${bakimdata.reason}** ~~`, bakimsonaerdikardesdisbots)
    })
