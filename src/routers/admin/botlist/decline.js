const app = require('express').Router();
const botsdata = require("../../../database/models/botlist/bots.js");
const emoji = require('../../../../emoji.json')
const codesSchema = require("../../../database/models/codes.js");
const uptimedata = require("../../../database/models/uptime.js");
const appsdata = require("../../../database/models/botlist/certificate-apps.js");
let sitedatalari = require("../../../database/models/analytics-site.js");

const roles = global.config.server.roles;
const channels = global.config.server.channels;
const client = global.Client;
const { MessageEmbed } = require("discord.js")

console.log("[Disbots.xyz]: Admin/Botlist/Decline Bot router loaded.");


app.post("/admin/decline/:botID", global.checkAuth, async (req, res) => {
    let rBody = req.body;
    let botdata = await botsdata.findOne({
        botID: req.params.botID
    });
    client.users.fetch(botdata.ownerID).then(sahip => {
        client.channels.cache.get(global.config.server.channels.botlog).send(new MessageEmbed()
        .setTitle(`${emoji.error} Bot Declined`)
       .setDescription(`${botdata.username} has been declined as a bot in our listing system.`)
       .addField(`Reason`, `${rBody['reason']}`)
    .addField(`Declined By`, `<@${req.user.id}>`)
    .addField(`Bot Name`, `${botdata.username}`)
    .addField(`Owner`, `${botdata.ownerID}`)
    .setColor('2f3136')
    )
        client.users.cache.get(botdata.ownerID).send(new MessageEmbed()
        .setTitle(`${emoji.error} Bot Declined`)
       .setDescription(`Your bot ${botdata.username} has been declined as a bot in our listing system.`)
       .addField(`Reason`, `${rBody['reason']}`)
    .addField(`Declined By`, `<@${req.user.username}>`)
    .setColor('2f3136')
    )
    })
    await botsdata.deleteOne({
        botID: req.params.botID,
        ownerID: botdata.ownerID
    })
    return res.redirect(`/admin/unapproved?success=true&message=Bot declined.`)
});

module.exports = app;