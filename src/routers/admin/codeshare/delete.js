...
app.post("/admin/deletecode/:code", global.checkAuth, async (req, res) => {
    ...
    client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
        .setTitle("Code deleted!").setColor("GREEN").setFooter(`Powered By DTL`)
        .setDescription(`The user named **[${req.user.username}](https://townlist.xyz/user/${req.user.id})** deleted the code named **${kodDataFind.codeName}**.`)
