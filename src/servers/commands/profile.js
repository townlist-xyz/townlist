...
module.exports.run = async (client, message, args) => {
  ...
  const embed = new Discord.MessageEmbed()
    .setTitle('DTL User Profile')
    .setAuthor(users.tag, users.avatarURL({ dynamic: true }))
    
    .setDescription(`\n[**Profile Link**](https://townlist.xyz/user/${users.id})\n\n**Analytics**\nYou own **${bots.length || "0"} Bots** and **${servers.length} Servers**`)
    .setColor("#2f3136")
    .addField("**Bots [" + bots.length + "]**", `${!bots ? "" : bots.map(a => "<@" + a.botID + ">  **[**[`Link`](https://townlist.xyz/bot/" + a.botID + ")**]**").join("\n")}\n\n**Servers [${servers.length}]** \n${!servers ? "" : servers.map(b => b.name + " **[**[`Link`](https://townlist.xyz/server/" + b.id + ")**]**").join("\n")}`, true)
